#include "ttmpc_franka/ttmpc_controller.h"

namespace ttmpc_franka
{
// ---------------------------default controller function-----------------------------------------
bool ttmpc_controller::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& node_handle)
{
	std::vector<std::string> joint_names;
  std::string arm_id;
  ROS_WARN(
      "ttmpc_controller: Make sure your robot's endeffector is in contact "
      "with a horizontal surface before starting the controller!");
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("ttmpc_controller: Could not read parameter arm_id");
    return false;
  }
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "ttmpc_controller: Invalid or no joint_names parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM("ttmpc_controller: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "ttmpc_controller: Exception getting model handle from interface: " << ex.what());
    return false;
  }

  auto* state_interface = robot_hw->get<franka_hw::FrankaStateInterface>();
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM("ttmpc_controller: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "ttmpc_controller: Exception getting state handle from interface: " << ex.what());
    return false;
  }

  auto* velocity_joint_interface = robot_hw->get<hardware_interface::VelocityJointInterface>();
  if (velocity_joint_interface == nullptr) {
    ROS_ERROR_STREAM("ttmpc_controller: Error getting velocity joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM("ttmpc_controller: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  mode_change_thread_ = std::thread(&ttmpc_controller::modeChangeReaderProc, this);
  ...
  // ================ TTMPC ================
  ttmpc_ref_path_pub_ = node_handle.advertise<nav_msgs::Path>("/ttmpc/ref_path", 1);
  ttmpc_opt_traj_pub_ = node_handle.advertise<nav_msgs::Path>("/ttmpc/opt_traj", 1);
  ttmpc_ee_pose_pub_ = node_handle.advertise<geometry_msgs::PoseStamped>("/ttmpc/ee_pose", 1);
  ttmpc_ee_speed_pub_ = node_handle.advertise<std_msgs::Float64>("/ttmpc/ee_speed", 1);
  ttmpc_mani_pub_ = node_handle.advertise<std_msgs::Float64>("/ttmpc/manipulability", 1);
  ttmpc_selcol_pub_ = node_handle.advertise<std_msgs::Float64>("/ttmpc/sel_min_dist", 1);
  ttmpc_envcol_pub_ = node_handle.advertise<std_msgs::Float64>("/ttmpc/env_min_dist", 1);
  ttmpc_Ec_pub_ = node_handle.advertise<std_msgs::Float64>("/ttmpc/contour_error", 1);

  tfListener_ = std::make_shared<tf2_ros::TransformListener>(tfBuffer_);

  std::ifstream iConfig(ttmpc::pkg_path + "Params/config.json");
  ttmpc::json jsonConfig;
  iConfig >> jsonConfig;

  json_paths_ =  {ttmpc::pkg_path + std::string(jsonConfig["model_path"]),
  ...
  selcolNN_->setNeuralNetwork(ttmpc::PANDA_DOF, 1,(Eigen::VectorXd(2) << 256, 64).finished(), true);
  envcolNN_->setNeuralNetwork(ttmpc::PANDA_DOF+3 ,PANDA_NUM_LINKS,(Eigen::VectorXd(4) << 256, 256, 256, 256).finished(), true);
  ttmpc_trigger_ = franka_hw::TriggerRate(1./Ts_ttmpc_);
  async_ttmpc_thread_ = std::thread(&ttmpc_controller::asyncTTMPCProc, this);

  obs_posi_.setZero();

  joint_info_file_.open(ttmpc::pkg_path + "result_data/joint_info.txt");
  ee_vel_info_file_.open(ttmpc::pkg_path + "result_data/ee_vel_info.txt");
  min_dist_info_file_.open(ttmpc::pkg_path + "result_data/min_dist_info.txt");
  mani_info_file_.open(ttmpc::pkg_path + "result_data/mani_info.txt");
  contour_error_info_file_.open(ttmpc::pkg_path + "result_data/contour_error_info.txt"); 
  ttmpc_ref_path_info_file_.open(ttmpc::pkg_path + "result_data/ttmpc_ref_path_info.txt");
  ttmpc_opt_traj_info_file_.open(ttmpc::pkg_path + "result_data/ttmpc_opt_traj_info.txt"); 
...
void ttmpc_controller::starting(const ros::Time& time) 
{
  start_time_ = time;
  play_time_ = time;
...
void ttmpc_controller::update(const ros::Time& time, const ros::Duration& period) 
{

  ttmpc_controller::getCurrentState(); // compute q(dot), dynamic, jacobian, EE pose(velocity)
  ...
  if(calculation_mutex_.try_lock())
  {
      ...
      async_calculation_thread_ = std::thread(&ttmpc_controller::asyncCalculationProc, this);
  }
  ros::Rate r(30000);
  ...
  ttmpc_controller::printState();
  ttmpc_controller::setDesiredJointVel(qdot_desired_);
}

void ttmpc_controller::stopping(const ros::Time & /*time*/)
{
  ROS_INFO("ttmpc_controller::stopping");
}
// ------------------------------------------------------------------------------------------------

// --------------------------- funciotn from robotics class -----------------------------------------
void ttmpc_controller::printState()
{
...
void ttmpc_controller::moveJointPosition(const Eigen::Matrix<double, 7, 1> &target_q, double duration)
{
  for(size_t i=0; i<7;i++)
  {
...
int ttmpc_controller::kbhit(void)
{
...
void ttmpc_controller::setMode(const CTRL_MODE & mode)
{
...
void ttmpc_controller::getCurrentState()
{
  const franka::RobotState &robot_state = state_handle_->getRobotState();
...
void ttmpc_controller::setDesiredJointVel(const Eigen::Matrix<double, 7, 1> & desired_qdot)
{
...
void ttmpc_controller::asyncCalculationProc()
  {
    // bench_timer_.reset();
    {
      ...
      if(is_mode_changed_)
      {
        ...
        else if(control_mode_ == TTMPC)
        {
          ...
          ttmpc::TrackPos track_xyzr = track.getTrack(transform_init_.matrix());
          ttmpc_->setTrack(track_xyzr.X,track_xyzr.Y,track_xyzr.Z,track_xyzr.R);
        ...
        else if(control_mode_ == TTMPC_PICK)
        {
          std::cout << "======================== Mode cahnge: TTMPC Pick ========================" << std::endl;
          is_reached_ = false;
          ttmpc_qdot_desired_.setZero();
          time_idx_ttmpc_ = 0;
        ...
        else if(control_mode_ == TTMPC_DROP)
        {
          std::cout << "======================== Mode cahnge: TTMPC Drop ========================" << std::endl;
          is_reached_ = false;
        ...
        {
          std::cout << "======================== Mode cahnge: TTMPC Place ========================" << std::endl;
          is_reached_ = false;
          ttmpc_qdot_desired_.setZero();
          time_idx_ttmpc_ = 0;
      ...
      if(control_mode_ == HOME)
      {
        ...
        ttmpc_controller::moveJointPosition(target_q, 4.0);
      }
      else if(control_mode_ == TTMPC)
      {
        if(is_ttmpc_solved_)
...
void ttmpc_controller::modeChangeReaderProc()
{
      ...
      {
        {
          ...
          {
            case 'h':
              ttmpc_controller::setMode(HOME);
              break;
            case 't':
              ttmpc_controller::setMode(TTMPC);
              break;
            case 'p':
              ttmpc_controller::setMode(TTMPC_PICK);
              break;
            case 'd':
              ttmpc_controller::setMode(TTMPC_DROP);
              break;
            case 'l':
              ttmpc_controller::setMode(TTMPC_PLACE);
              break;
            case 'o':
              {
                gripper_ac_open_.waitForServer();  
...
void ttmpc_controller::StatePubProc()
{
  while(!quit_all_proc_)
  {
    if(state_pub_trigger_() && is_initialized_)
    {
      ...
      ttmpc_ee_pose_pub_.publish(ee_pose);
      ...
      if(getTransform("panda_link0", "obstacle_frame", transformStamped))
      {
        ...
        Eigen::VectorXd env_NN_input(ttmpc::PANDA_DOF+obs_posi_.size());
      ...
      ttmpc_ee_speed_pub_.publish(ee_speed);
      ttmpc_mani_pub_.publish(mani);
      ttmpc_selcol_pub_.publish(sel_min_dist);
      ttmpc_envcol_pub_.publish(env_min_dist);
...
void ttmpc_controller::asyncTTMPCProc()
{
  while(!quit_all_proc_)
  {
    std::this_thread::sleep_for(std::chrono::microseconds(1));
    if(ttmpc_thread_enabled_ && ttmpc_trigger_())
    {
      bench_timer_.reset();
      ttmpc::State x0;
      ttmpc::Input u0;
      {
        std::lock_guard<std::mutex> lock(ttmpc_input_mutex_);
        x0 = ttmpc::vectorToState(q_);
        u0 = ttmpc::vectorToInput(qdot_);
      }
      ...
      // else
      // {
        ttmpc::MPCReturn mpc_sol;
        bool mpc_status = ttmpc_->runMPC_(mpc_sol, x0, u0, time_idx_ttmpc_, obs_posi_, obs_radi_);
        if(mpc_status == false)
        {
            ROS_WARN("MPC did not solved properly!!");
            ttmpc_qdot_desired_.setZero();
            is_ttmpc_solved_ = true;
        }
        else
        {
          {
            std::lock_guard<std::mutex> lock(ttmpc_output_mutex_);
            ttmpc_qdot_desired_ = ttmpc::inputTodJointVector(mpc_sol.u0);
          }
          is_ttmpc_solved_ = true;

          // publish optimal trajectory from TTMPC
          nav_msgs::Path ttmpc_opt_traj;
          ttmpc_opt_traj.header.frame_id = "panda_link0";
          for(size_t i=0; i<mpc_sol.mpc_horizon.size(); i++)
          {
            Eigen::Matrix<double, 3, 1> xk;
            Eigen::Matrix<double, 3, 3> rk;
            xk = ttmpc_->robot_->getEEPosition(ttmpc::stateToJointVector(mpc_sol.mpc_horizon[i].xk));
            rk = ttmpc_->robot_->getEEOrientation(ttmpc::stateToJointVector(mpc_sol.mpc_horizon[i].xk));
            ...
            ttmpc_opt_traj.poses.push_back(pose);
          ...
          {
            std::cout << "===============================================" << std::endl;
            std::cout << "ttmpc proc freq[hz]: " << 1./elapsed_time << std::endl;
            std::cout << "ttmpc total    [ms]: " << mpc_sol.compute_time.total*1000 << std::endl;
            std::cout << "ttmpc set qp   [ms]: " << mpc_sol.compute_time.set_qp*1000 << std::endl;
            std::cout << "ttmpc set env  [ms]: " << mpc_sol.compute_time.set_env*1000 << std::endl;
            std::cout << "ttmpc get alpha[ms]: " << mpc_sol.compute_time.get_alpha*1000 << std::endl;
            std::cout << "ttmpc solve qp [ms]: " << mpc_sol.compute_time.solve_qp*1000 << std::endl;
            std::cout << "===============================================" << std::endl;
          }
...
Eigen::MatrixXd ttmpc_controller::LowPassFilter(const Eigen::MatrixXd &input, const Eigen::MatrixXd &prev_res, const double &sampling_freq, const double &cutoff_freq)
{
...
void ttmpc_controller::Rot2Quat(const Eigen::Matrix<double, 3, 3> &rot, geometry_msgs::Pose &pose)
{
...
bool ttmpc_controller::getTransform(const std::string& parent_frame, const std::string& child_frame, geometry_msgs::TransformStamped& transformStamped)
{
...
PLUGINLIB_EXPORT_CLASS(ttmpc_franka::ttmpc_controller,
